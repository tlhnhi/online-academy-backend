...
router.get("/verify/:token", (req, res) => {
  try {
    const userToken = req.params.token || null;
    token.verifyToken(userToken, async (err, payload) => {
      if (err) return handleError(res, err, "Get error");
      const user = await UserModel.findById(payload.sub).select("_id verified");
      if (user === null) sendResponse(res, false, "Get error");
      user.verified = true;
      await user.save();
    });
    return sendResponse(res, true, "Verified successfully.");
  } catch (error) {
    return handleError(res, error, "Get error");
  }
});
...
router.post("/verify", async (req, res) => {
  try {
    ...
    if (user === null) sendResponse(res, false, "Invalid user email");

    var userToken = token.generateToken(user);
    sendVerify(userToken, email);

    return sendResponse(res, true, "Email has been sent.");
  } catch (error) {
    return handleError(res, error, "Get error");
  }
});
...
router.get("/category", async (req, res) => {
  try {
    const parents = await CategoryModel.find({ parent: null }).select("-__v");
    let parentObjs = [],
      parentObj;
    for (let k = 0; k < parents.length; k++) {
      ...
      for (let i = 0; i < category.length; i++) {
        ...
        courses = await CourseModel.find({ category_id: obj._id }).select(
          "_id"
        );
        obj.courses = courses.length;
        obj.enrollments = 0;
        for (let j = 0; j < courses.length; j++) {
          enrollments = await UserModel.find({
            enrolled: courses[j]._id
          }).select("_id");
...
router.get("/category/:id", async (req, res) => {
  try {
    ...
    const courses = await CourseModel.find({ category_id: id }).select(
      "-content -category_id -__v"
    );
...
router.get("/course/:id", async (req, res) => {
  try {
    ...
    for (let i = 0; i < course.rating.length; i++) {
      ...
      courseObj.rating[i].user = await UserModel.findById(
        courseObj.rating[i].user_id
      ).select("-password -__v");
    ...
    courseObj.lecturer = await UserModel.findById(courseObj.lecturer_id).select(
      "-password -__v"
    );
    delete courseObj.lecturer_id;
    courseObj.category = await CategoryModel.findById(
      courseObj.category_id
    ).select("-__v");
...
router.post("/search", async (req, res) => {
  try {
    ...
    let courses;
    if (keyword)
      courses = await CourseModel.find({ $text: { $search: keyword } }).select(
        "-content -__v"
      );
