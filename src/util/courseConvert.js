...
export const convert = async courses => {
  ...
  for (let i = 0; i < courses.length; i++) {
    ...
    // Calculate star
    star = 0;
    for (let j = 0; j < obj.rating.length; j++) star += obj.rating[j].star;
    ...
    if (obj.lecturer_id !== undefined) {
      obj.lecturer = await UserModel.findById(obj.lecturer_id).select(
        "-password -__v"
      );
    ...
    if (obj.category_id !== undefined) {
      obj.category = await CategoryModel.findById(obj.category_id).select(
        "-__v"
      );
